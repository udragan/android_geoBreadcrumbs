...
class DistanceTransformation(val context: Context) :
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String) {
        if (key == context.getString(R.string.settings_preference_measurement_unit_key)) {
            LogEx.d(Constants.TAG_TRANSFORMATION_DISTANCE, "preference changed")
            val defaultValue = context.resources.getStringArray(R.array.measurement_unit_values)[0]
    ...
    fun transform(distance: Float): String {
        val unitSystem = context.resources.getStringArray(R.array.measurement_unit_values)
    ...
    private fun readExistingPreference() {
        val defaultValue =
            context.resources.getStringArray(R.array.measurement_unit_values)[0]
